...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($balance === '' || !is_numeric($balance)) $errors[] = "Le solde doit être un nombre.";

    if (empty($errors)) {
        $stmt = $pdo->prepare("
            UPDATE accounts
            SET account_number = ?, type = ?, status = ?, balance = ?
            WHERE id = ?
        ");
        $stmt->execute([$account_number, $type, $status, $balance, $id]);
